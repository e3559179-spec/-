...
- 第四阶段工具用 `.current.json` 指向完整快照，顶层两份 JSON 为可查看的镜像。保存元数据由工具生成；显式恢复旧快照会形成新版本，`previous_save_id` 指向被恢复的快照，并追加恢复原因。业务字段仍为版本 1。
...
产生问题、解释、理解反馈或进度变化后，在本轮结束前及时保存；会话结束时额外更新续读摘要，不依赖结束口令才首次保存。具体命令、提交语义和恢复步骤统一见 [持久保存与续读](persistence-workflow.md)。

先 `load`，基于完整对象形成两份候选，保留原话、未知字段和既有历史；调用 `save` 时传入读取时的保存 ID 和状态令牌。工具自动执行理解记录校验、来源与问题引用检查、版本冲突检查及回读验证。内容含义、用户确认依据和变化历史的真实性仍需助手核对，不能把工具成功当作这些语义判断正确。

成功结果须同时满足退出码 0、`ok: true` 和 `mirror_state: consistent`，再说明保存路径。失败先保留候选并读取真实状态，不报告成功、不盲目重试覆盖；没有工具时交付完整待保存文件，不能用聊天记忆代替持久保存。

## 恢复与续读

先使用当前明确的数据目录；换工作目录且不知道路径时，请用户提供旧路径或记录，不从聊天记忆猜测，也不扫描整个设备。

- 采用快照工具的目录：读取当前指针所指的校验有效快照；镜像中断、手工修改冲突和指针/快照损坏分别按工具流程处理，不仅凭两份镜像 ID 相同判断最新版本。
- 原先只有两份 JSON 的旧目录：核对一致性及版本后用 `import-legacy` 导入，保留原始字节；有快照目录但缺少指针不能直接当作旧目录覆盖。
- 原文不可访问时说明需要原文，记录本身不证明正文仍可读。有多个恢复候选且无法判断时，呈现差异并询问选择，不擅自用旧数据覆盖新数据。
- 完全没有记录：说明未找到；根据用户提供的当前位置重新建立，不能写成已恢复历史。
- 续读摘要不能替代完整记录。总结前读取问题、解释依据、变化历史和覆盖范围；没有检查记录的章节必须明确注明。

工具不会自动安排会话、搜索原文或跨设备同步。后续每次伴读仍需由运行助手在发生变更时调用保存，并在换对话后提供可访问的数据目录。
